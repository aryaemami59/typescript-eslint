...
describe('Validating rule docs', async () => {
  ...
  describe.each(rulesData)(`%s.mdx`, async (ruleName, rule) => {
      ...
      describe('rule options', () => {
        ...
        for (const schemaItem of schema) {
          if (schemaItem.type === 'object') {
            const testCases = Object.keys(schemaItem.properties as object);

            test.for(testCases)(`%s`, property => {
    ...
    test.todo('code examples ESLint output', () => {
      ...
      unistUtilVisit.visit(tree, node => {
        ...
        if (node.type === 'code') {
          if (node.meta?.includes('showPlaygroundButton')) {
            void lintCodeBlock(node, 'skip-check');
          }
