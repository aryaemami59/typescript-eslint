...
describe('Validating rule docs', async () => {
  ...
  describe.for(rulesData)('%s.mdx', async ([ruleName, rule]) => {
    ...
    test('code examples ESLint output', async ({ expect }) => {
      ...
      unistUtilVisit.visit(tree, node => {
        ...
        if (node.type === 'code') {
          if (node.meta?.includes('showPlaygroundButton')) {
            targetedTokens.push([node, 'skip-check']);
          }
      ...
      const snapshotContents = targetedTokens
        .map(([token, shouldContainLintErrors]) =>
          lintCodeBlock(token, shouldContainLintErrors),
        )
        .filter(snapshotContent => snapshotContent != null);
