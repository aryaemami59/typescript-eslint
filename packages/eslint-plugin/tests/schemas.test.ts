...
describe('Rule schemas should be convertible to TS types for documentation purposes', async () => {
  ...
  describe.each(testCases)('%s', (ruleName, ruleDef) => {
...
test('There should be no old snapshots for rules that have been deleted', async () => {
...
describe('Rule schemas should validate options correctly', () => {
  ...
  it.for(testCases)('%s must accept valid options', ([ruleName, rule]) => {
    if (
      !areOptionsValid(
        rule,
  ...
  it.for(testCases)('%s rejects arbitrary options', ([_ruleName, rule]) => {
